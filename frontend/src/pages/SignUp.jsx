...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // After successful signup, user needs to set up their profile (username).
      // The backend sets the auth cookie, so we can navigate to the setup page.
      // The AuthProvider will then pick up the user on the next page load and
      // confirm they need a username.
      ...
      <div className="max-w-md w-full space-y-8 p-8 rounded-xl border-2 border-level-2 bg-level-1">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Join BrightPath to start your learning journey
          </p>
        </div>
        ...
        <p className="text-center text-sm text-gray-600 dark:text-gray-400">
          ...
          <Link to="/signin" className="font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400 hover:border-b-2 hover:border-primary">
            Sign in
          </Link>
        </p>
      </div>
